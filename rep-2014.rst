...
Results from benchmarking are only meaningful if they are _reproducible_, that is, the benchmark can be repeated with the exact same results, or very close to it. This starts with the workload definition: The workload must be exactly described, and any possibly interfering effects should be removed. 

A challenge here is that, surprisingly often, interfering effects are not fully known. For example, many current computer systems try to optimize power usage, or handle heat issues, by throttling the CPU. This effect is often not captured, and would cause any resultings measurements to be not reproducible. Similarly, on many Linux systems a number of system processes are running which also consume resources. This is sometimes tried to account for by only measuring CPU usage of the system-under-test. However, there are shared CPU resources, such as caches, or when using hyper-threading, where any other process can affect the benchmark.

Therefore, it is imperative that the measures taken to remove interfering effects are exactly specified, so that they can, firstly, be followed when replicating the benchmark, and secondly, also examined for omissions. Since knowledge of such things is constantly evolving, we recommend examining current literature on benchmarking to identify best practices for removal of interfering effects and description of setups. The description in [15] is a good, recent example.
...
.. [15] C. Plasberg, H. Nessau, D. Timmermann, C. Eichmann, A. Roennau and R. Dillmann, "Towards Distributed Real-Time capable Robotic Control using ROS2," 2022 IEEE 18th International Conference on Automation Science and Engineering (CASE), 2022, pp. 2205-2210, doi: 10.1109/CASE49997.2022.9926569.
